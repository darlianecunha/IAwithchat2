...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    if (!file) return res.status(400).json({ error: "Audio is required (field 'audio')" });
    ...
    const parts: any[] = [{ text: instruction || "Transcribe and respond briefly in PT-BR." }];
    parts.push({ inlineData: { mimeType: mime || "audio/webm", data: file.toString("base64") } });
